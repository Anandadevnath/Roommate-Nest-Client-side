...
const AllItems = () => {
  ...
  const fetchTrending = async () => {
    try {
      const response = await fetch('http://localhost:5000/trending?limit=6');
      const data = await response.json();
      setTrending(data);
    } catch (error) {
  ...
  const fetchSearchSuggestions = async () => {
    try {
      const response = await fetch(`http://localhost:5000/search-suggestions?query=${encodeURIComponent(searchTerm)}`);
      const data = await response.json();
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/all-items?${queryParams}`);
      if (!response.ok) {
