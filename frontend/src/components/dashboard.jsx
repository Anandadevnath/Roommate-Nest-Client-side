...
const Dashboard = () => {
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch(`http://localhost:5000/dashboard/stats?email=${encodeURIComponent(user.email)}`);
      if (response.ok) {
  ...
  const fetchMyListings = async () => {
    try {
      const response = await fetch(`http://localhost:5000/my-listings?email=${encodeURIComponent(user.email)}`);
      if (response.ok) {
        const data = await response.json();
        setMyListings(data);
      }
  ...
  const fetchAllListings = async () => {
    try {
      const response = await fetch('http://localhost:5000/all-items?limit=20');
      if (response.ok) {
        const data = await response.json();
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`http://localhost:5000/roommates/${id}`, { method: 'DELETE' });
        if (response.ok) {
  ...
  const handleLike = async (id) => {
    try {
      const response = await fetch(`http://localhost:5000/roommates/${id}/like`, {
        method: 'PATCH'
      });
      if (response.ok) {
        const data = await response.json();
